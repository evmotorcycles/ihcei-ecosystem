...
class TestNEREPilot(unittest.TestCase):
    ...
    def test_nere_auditor_init(self, mock_rfc, mock_st):
        # Setup mocks
        ...
        self.assertTrue(mock_rfc.return_value.fit.called)

    def test_execute_pilot_math(self):
        ...
        nere_pilot.execute_pilot(self.dataset, mock_auditor, traditional_prompt, nere_prompt)
        ...
        actual_trad = self.dataset.iloc[0]['E_Trad_Recovered']
        actual_nere = self.dataset.iloc[0]['E_NERE_Recovered']

        # Using almost equal for float comparison
        self.assertAlmostEqual(actual_trad, expected_trad, places=5)
        self.assertAlmostEqual(actual_nere, expected_nere, places=5)

        # Verify entire columns
        pd.testing.assert_series_equal(
            self.dataset['E_Trad_Recovered'],
            self.dataset['U_Raw_Utility'] * expected_trad_factor,
            check_names=False
        )
        pd.testing.assert_series_equal(
            self.dataset['E_NERE_Recovered'],
            self.dataset['U_Raw_Utility'] * expected_nere_factor,
            check_names=False
        )
