...
const P = {
  ...
  g4: [/\bbypass\b/i, /\bskip\s+the\b/i, /\bno\s+need\s+to\s+(check|verify|review)\b/i, /\bdon'?t\s+(need|have)\s+to\s+verify\b/i, /\bignore\s+the\s+procedure\b/i, /\bjust\s+trust\b/i],
  ...
  g6: [/\bas\s+you\s+know\b/i, /\bthe\s+complexity\s+here\b/i, /\bthis\s+is\s+very\s+technical\b/i, /\btoo\s+complex\s+to\s+explain\b/i],
...
function extractEvidenceFast(text) {
  ...
  return { hits, urg, fear, opt, imp, meth, T, deltaA, g3s, g7s };
}
...
function posterior(ev, priorP = 0.10, nMc = 3000, seed = 7) {
  ...
  for (const [gid, g] of Object.entries(GATES)) {
    ...
    if (id === 3) { contrib = g.llr * ev.g3s; sd = g.sd * Math.max(ev.g3s, 0.2); hits = ev.g3s > 0 ? 1 : 0; }
    else if (id === 7) { const on = ev.g7s > 0.30; contrib = on ? g.llr * ev.g7s : 0; sd = on ? g.sd * Math.max(ev.g7s, 0.2) : 0; hits = on ? 1 : 0; }
  ...
  const extras = [['urgency', ev.urg], ['fear', ev.fear], ['imperatives', ev.imp], ['options', ev.opt], ['methodology', ev.meth]];
  for (const [k, c] of extras) {
    const s = EXTRA[k]; const eff = Math.min(c, s.cap);
    if (eff) {
      const scale = s.llr > 0 ? eff : Math.sqrt(eff);
      terms.push([s.llr * scale, s.sd * Math.sqrt(eff)]);
      evidence.push({ gate: k, hits: c, llr: +(s.llr * scale).toFixed(3) });
    }
  }
  const r = rng(seed);
  ...
  return { mean, ci, evidence, totalLlr: terms.reduce((a, [m]) => a + m, 0) };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const payload = `${Date.now()}|${verdict}|${post.mean.toFixed(4)}`;
    let h = 0; for (const c of payload) h = (h * 31 + c.charCodeAt(0)) >>> 0;
    return res.status(200).json({
      ...
      correction,
      certificate: 'GOV-' + h.toString(16).toUpperCase().padStart(8, '0'),
      engine,
