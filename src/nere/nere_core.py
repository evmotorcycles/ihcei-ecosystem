...
class NERECore:
    ...
    def audit_decision(self, context: str, decision_data: Dict[str, Any],
        ...
        # Apply Heuristic Overlay (Simulated Intelligence for Demo)
        h_shirk, h_riba, h_compliance = self._heuristic_check(context)

        # Override random model weights with heuristic logic for the demonstration
        shirk_score = h_shirk
        riba_score = h_riba
        alignment_score = h_compliance
        element_scores = np.full(10, h_compliance)
    ...
    def _generate_embeddings(self, context: str, data: Dict[str, Any]) -> np.ndarray:
        ...
        For simulation, we use a keyword-based heuristic to generate embeddings
        that will trigger the NERE model (which is currently random weights)
        in a predictable way, or we intercept the scoring.

        Since the NEREModel has random weights and we can't train it here,
        we will rely on a heuristic 'embedding' that we know pushes the
        untrained model (or a heuristic bypass) in the right direction.

        Actually, relying on random weights to process these embeddings is futile.
        We should override the audit logic for the simulation to use keywords directly
        if we want valid demonstration results without training.
        """
        ...
        embedding += np.random.normal(0, 0.01, 768) * 0.01

        return embedding

    def _heuristic_check(self, context: str) -> Dict[str, float]:
        """
        A heuristic check to simulate a trained model's intuition.
        This allows the pilot and audit scripts to function meaningfully
        without a trained BERT model.
        """
        context_lower = context.lower()

        # Keywords for Corruption/Shirk
        shirk_keywords = ['ignore rules', 'absolute authority', 'unquestionable', 'no questions', 'tradition dictates']
        shirk_score = 0.05
        for kw in shirk_keywords:
        ...
        # Keywords for Fitnah/Bad Faith
        fitnah_keywords = ['spread rumors', 'destroy reputation', 'without evidence']
        for kw in fitnah_keywords:
             if kw in context_lower:
                 shirk_score += 0.3 # Treat as form of corruption

        # Compliance (defaults to high, reduced by bad stuff)
        compliance = 0.95
        if shirk_score > 0.1 or riba_score > 0.1:
            compliance = 0.4

        return shirk_score, riba_score, compliance
