...
import pytest
import numpy as np
from src.core.qg_cos_cgmm_optimized import GovernancePhysics, ADGENetwork, CGMM_Agent

class TestGovernancePhysicsOptimized:
    """
    Tests for the GovernancePhysics class (Optimized).
    """
...
class TestADGENetworkOptimized:
    """
    Tests for the ADGENetwork class (Optimized).
    """
    ...
    def test_adge_network_diagonal_zero(self):
        """
        Test that self-interaction (diagonal) is ignored even if alignment is set.
        """
        network = ADGENetwork(num_agents=2)
        network.set_agent_stage(0, 10)
        network.set_alignment(0, 0, 1.0) # Self alignment

        # Should be ignored
        c_dev = network.calculate_c_dev(open_gates_of_entropy=0)
        assert c_dev == pytest.approx(0.0)

class TestCGMMAgentOptimized:
    """
    Tests for the CGMM_Agent class (Optimized).
    """
